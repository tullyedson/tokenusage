...
`models()` returns `Vec<InferenceModel>`, with an exact `id` and `ModelLimits { context, input, output }` optional token counts. Return verified provider/server bounds, never limits inferred from a model name or a model's output maximum used as context. Unknown values remain `None`. Native `/v1/models` fields are parsed by `ModelLimits::from_catalog`; any supplemental source belongs inside the adapter. Core computes each pool field as the minimum across every enabled eligible member, with any unknown or stale member making that field unknown. Do not filter depleted accounts out of that calculation. Missing model metadata must not prevent an otherwise valid model name from being listed. Preserve metadata through the generic catalog, native Models page and `/v1/models` response.

Go and Ollama Cloud share a bounded, credential-free Models.dev cache containing only their exact provider/model limits. Ollama Cloud's native `/api/show` context takes precedence. Local vLLM uses its served `max_model_len`; local Ollama requires the model's explicit `num_ctx` parameter because its trained maximum does not reveal the server default. Metadata enrichment is bounded and cancelled with its owner. Avoid metadata supplement requests on each generation preflight; catalog metadata refresh and spending eligibility are separate concerns. Add parser, mixed-chain, unknown/stale-bound and client-import regressions for a new metadata source.

Inference is separate from usage monitoring. A query adapter must enforce local/free/included-only spending before it is registered. Never use subscription meters as authorization for an unrelated paid API key. Check every applicable quota window for the requested upstream model; return `Unavailable` with a provider reset/retry time and account/model scope. An expired reset permits another check, not an invented fresh allowance. If enforcement or allowance status cannot be established, fail closed. The HTTP adapters demonstrate verified model catalogs, loopback/private server validation, including connection-time private-address resolution for .local hostnames and a provider-side zero-price guard.

The common transport speaks OpenAI-compatible chat completions. Bounded JSON and SSE forwarding keep the requested pool name in the response model field and expose the selected upstream ID in a response header. ModelPool owns a route mode and an ordered list of accountId/model pairs. The engine applies ordered failover or sticky load distribution without adapter-specific branches. The distribution reservation lives through preflight, queueing and the entire response, including streams. Every eligible member remains part of the minimum context/output calculation for either mode. Only edited pools are persisted; exact model names are otherwise grouped automatically. No provider branches or per-account model mappings are needed. `InferenceContext` also supplies the request clock and optional validated session ID. `PreparedRequest.headers` carries only adapter-owned, allowlisted protocol headers; never forward arbitrary caller headers or the router's client key. A provider using a different response protocol needs a tested conversion at the inference boundary. Do not pass native Responses/Messages output through as chat completions. Native agent runtimes are not generic chat HTTP endpoints.
...
Local adapters must use `routing::network::client_builder()` for both discovery/usage and inference. The shared resolver validates every address returned for a .local host and passes those exact addresses to the connection. Do not validate a hostname with one lookup and allow the transport to resolve it again, and do not use an unguarded HTTP client for local DNS names. Read-only preflight failures can return Unavailable; a failure after generation may have been submitted must retain the no-replay rule.

## Optional call measurements

The router measures incoming JSON body size and accepted upstream response body size independently of adapters. `routing/metrics.rs` parses the standard Chat Completions `usage` counters from JSON and SSE; keep these counters in the upstream response. Missing counts stay unknown. Compatible adapters can call `request_stream_usage` to default `include_usage` on while preserving caller options. Do not add this option to a provider that rejects it.

For an account allowance observation, populate `PreparedRequest.allowance_before` with a numeric `AllowanceSnapshot` from an existing eligibility read, or `None`. Implement the optional `IInferenceProvider::observe_allowance` to read the same account through its established connection and return numeric windows with stable IDs, safe static labels, percent used and absolute reset timestamps. The default is `None`. Never pass a raw response, account identity, prompt, tool arguments or credentials into report data. OpenCode Go is the reference implementation in `routing/subscriptions.rs`.

The engine owns the bounded background read after response completion, cancellation, comparison, report update and retention rules. It does not turn an observation into billing permission or quota eligibility. Match windows and resets, and label the result as observed percentage-point change, never exact per-call allocation. Test missing/invalid counters, resets, cancellation, delayed reads, streaming cumulative totals and history clearing with fictional HTTP fixtures. No service or frontend provider-ID branches are needed.

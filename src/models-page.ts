...
export class ModelsPage {
  ...
  private automatic(): ModelPool[] {
    ...
    for (const catalog of this.library.catalogs) for (const { id: model } of catalog.models) {
      const pool = pools.get(model) ?? { name: model, mode: "failover", members: [] };
  ...
  private bind(root: HTMLElement): void {
    ...
    root.querySelector<HTMLFormElement>("[data-new-pool]")?.addEventListener("submit", event => {
      ...
      this.custom.push({ name, mode: "failover", members: [] }); this.selected = name; this.opened.add(name); this.dirty = true; this.render();
    });
    ...
    root.querySelectorAll<HTMLDetailsElement>("[data-pool]").forEach(card => {
      ...
      card.querySelector<HTMLSelectElement>("[data-pool-mode]")?.addEventListener("change", event => {
        if (this.saving) return;
        const mode = (event.currentTarget as HTMLSelectElement).value;
        const pool = this.edit(name);
        if (pool && (mode === "failover" || mode === "loadDistribution")) pool.mode = mode;
        this.opened.add(name); this.render();
      });

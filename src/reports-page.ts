...
import { metricsSummary, renderMetrics } from "./report-metrics";
...
function routeType(row: RequestReport): string { return row.mode === "loadDistribution" ? "Load distribution" : "Failover"; }
...
export class ReportsPage {
  ...
  mount(container: HTMLElement, data: Bootstrap): void {
    ...
    container.innerHTML = `<section class="page-heading"><div><span class="eyebrow">FOLLOW YOUR REQUESTS</span><h1>Reports</h1><p>See which pipeline is working and where it goes.</p></div><button class="secondary" id="refresh-reports">↻ Refresh</button></section>
      ...
      <section class="report-section"><div class="report-section-heading"><div><h2>Recent requests</h2><p>Expand a request for its route, sizes, tokens and usage percentages.</p></div><button class="text-button" id="clear-routing-history">Clear history</button></div>
      ...
      </section><p class="report-privacy" id="report-retention">Routing metadata stays in memory on this PC. Prompt and response content, headers and keys are never recorded. History clears when AI Usage exits.</p>`;
  ...
  private paint(): void {
    ...
    if (live) live.innerHTML = active.length ? active.map(row => `<article class="report-active-card" data-active-request="${esc(row.id)}"><div class="report-card-top"><span class="request-status ${row.status}">${esc(labels[row.status])}</span><span>#${esc(row.id)} · ${duration(row.durationMs)}</span></div><h3><code>${esc(row.pool)}</code><small class="report-route-type">${routeType(row)}</small></h3><div class="report-destination"><span class="route-arrow" aria-hidden="true">↓</span><div>${row.target ? this.destination(row.target) : '<span class="report-account">Choosing an eligible entry</span>'}</div></div><p>${esc(row.message)}</p>${renderMetrics(row)}<div class="report-card-bottom"><span>${row.streaming ? "Streaming request" : "JSON request"}</span><span>${row.target ? `Entry ${row.target.position}${row.fallbackCount ? " · fallback" : row.mode === "loadDistribution" ? " · assigned" : " · preferred"}` : "Checking pool"}</span></div></article>`).join("") : '<p class="report-empty">No requests in progress. Start a request from a connected app to see its pipeline here.</p>';
    const retained = this.container.querySelector("#report-retention");
    if (retained) retained.textContent = `Counts describe the last ${historyLimit} finished requests. Routing metadata and numeric measurements stay in memory on this PC and clear when AI Usage exits. Prompt and response content, headers and keys are never recorded.`;
  ...
  private paintRecent(): void {
    ...
    list.innerHTML = rows.length ? rows.map(row => `<details class="request-report" data-request="${esc(row.id)}" ${this.expanded.has(row.id) ? "open" : ""}><summary><span class="report-request-time">${esc(time(row.startedAt))}<small>#${esc(row.id)}</small></span><span class="report-request-route"><code>${esc(row.pool)}</code><small class="report-route-type">${routeType(row)}</small><small>${row.target ? `${esc(this.provider(row.target))} · ${esc(row.target.accountLabel || row.target.accountId)} → ${esc(row.target.model)}` : "No provider served this request"}</small></span><span class="request-result"><span class="request-status ${row.status}">${esc(labels[row.status])}</span><small>${duration(row.durationMs)}${row.fallbackCount ? ` · ${row.fallbackCount} fallback${row.fallbackCount === 1 ? "" : "s"}` : ""}</small>${metricsSummary(row)}</span><span class="report-chevron" aria-hidden="true">⌄</span></summary><div class="report-detail"><p>${esc(row.message)}${row.httpStatus !== null ? ` <span class="report-http">HTTP ${row.httpStatus}${row.streaming ? " (connection response)" : ""}</span>` : ""}</p>${renderMetrics(row)}${row.omittedAttempts ? `<p class="report-muted">${row.omittedAttempts} earlier steps omitted. Showing the last ${this.report?.attemptLimit} steps.</p>` : ""}<ol class="report-attempts">${row.attempts.map(attempt => `<li class="attempt-${attempt.outcome}"><span class="attempt-position">${attempt.target.position}</span><div><strong>${esc(this.provider(attempt.target))} · ${esc(attempt.target.accountLabel || attempt.target.accountId)}</strong><code>${esc(attempt.target.model)}</code><p><span class="attempt-outcome">${attempt.outcome === "selected" ? "Selected" : attempt.outcome === "skipped" ? "Skipped" : "Failed"}.</span> ${esc(attempt.reason)}${attempt.retryAt !== null ? ` <span>Check again ${esc(new Date(attempt.retryAt * 1000).toLocaleString())}.</span>` : ""}</p></div></li>`).join("")}</ol>${!row.attempts.length ? '<p class="report-muted">No provider accepted this request.</p>' : ""}</div></details>`).join("") : `<p class="report-empty">${this.report.recent.length ? "No requests match these filters." : "No finished requests yet. Reports start with requests received after this app launch."}</p>`;
